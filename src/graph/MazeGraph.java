	...
	public MazeGraph(Maze maze) {
		for(int x = 0; x < maze.getMazeWidth(); x++) {
			for(int y = 0; y < maze.getMazeHeight(); y++) {
				this.addVertex(new Juncture(x,y));		//adds vertex for each juncture
			}
		}
		for(int x = 0; x < maze.getMazeWidth(); x++) {
			for(int y = 0; y < maze.getMazeHeight(); y++) {
				...
				if(!maze.isWallAbove(j)) {
					this.addEdge(j, new Juncture(x,y-1), maze.getWeightAbove(j)); 	//adds edge if no wall above
				}
				if(!maze.isWallBelow(j)) {
					this.addEdge(j, new Juncture(x,y+1), maze.getWeightBelow(j));	//adds edge if no wall below
				}
				if(!maze.isWallToLeft(j)) {
					this.addEdge(j, new Juncture(x-1,y), maze.getWeightToLeft(j));	//adds edge if no wall to left
				}
				if(!maze.isWallToRight(j)) {
					this.addEdge(j, new Juncture(x+1,y), maze.getWeightToRight(j));	//adds edge if no wall to right
				}
			}
		}
	}
